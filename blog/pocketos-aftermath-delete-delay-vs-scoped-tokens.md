...
That is the gap an agent-side runtime gate can cover. Application policy classifies and authorizes the proposed call before it leaves the harness. Cycles can separately reserve caller-assigned exposure: a live reservation succeeds with `ALLOW` or configured `ALLOW_WITH_CAPS`, or returns a budget error. `DENY` with a `reason_code` is used by `decide` and dry-run flows.
