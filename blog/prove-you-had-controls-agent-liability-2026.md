...
Teams running [runtime authority](/glossary#runtime-authority) get scoped reserve and settlement records as a side effect. They must separately retain non-persisting preflight decisions, live rejection responses, identity-policy decisions, tool arguments, and action outcomes; the [evidence layer](/blog/audit-evidence-has-to-survive-production) can add signed, content-addressed receipts when enabled. That reframes the cost conversation: enforcement infrastructure can support both incident prevention and later proof, but only as one component of a complete evidence system.
