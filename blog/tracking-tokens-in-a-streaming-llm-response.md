---
title: "Track Tokens in Streaming LLM Responses"
date: 2026-04-19
...
**Release-on-exception defaults to zero-cost attribution.** If the provider already started generating tokens before the disconnect, those tokens were paid for at the provider — but the release path doesn't record them as spend. An `actual_cost` override is only read by the clean-exit commit path. To account for partial generation with `StreamReservation`, catch the provider exception inside the `with`, set `reservation.usage.actual_cost`, allow a clean exit, then propagate the saved exception after the commit. Use an explicit lifecycle instead if your application cannot safely convert that exceptional exit into a commit boundary.
