...
- **Risk coverage:** pre-execution when the application classifies the action, assigns `RISK_POINTS`, and submits the reservation before the mandatory boundary. A high-consequence action such as `delete_*` can consume more of a caller-provisioned exposure budget than `read_file`; Cycles does not infer the tier from the action name. See [How Cycles Meters Caller-Assigned Action Exposure](/blog/beyond-budget-how-cycles-controls-agent-actions) for the composed budget-and-authorization framing.
...
- [How Cycles Meters Caller-Assigned Action Exposure](/blog/beyond-budget-how-cycles-controls-agent-actions) — the foundational "cost is one axis" post; separates [action authority](/glossary#action-authority) from [budget authority](/glossary#budget-authority).
