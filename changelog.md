...
- **v0.1.25.63** (2026-06-22) — **Reservation evidence links.** Reservation detail now shows one-click reserve / commit / release evidence links and list calls request the runtime `include=evidence` projection. Requires `cycles-server` v0.1.25.37+; older servers omit the field and no links render.
- **v0.1.25.62** (2026-06-22) — **Reservations and Evidence gap closure.** Adds created/expires/finalized filters, reservation metadata detail, policy/webhook advanced config, API key/tenant fields, and an Evidence page that retrieves `GET /v1/evidence/{id}` and resolves signer keys against the runtime JWKS. The dashboard proxy now needs runtime routes for `/v1/reservations*`, `/v1/evidence*`, and `/v1/.well-known/cycles-jwks.json`.
...
|---|---|
| Dashboard Evidence viewer and reservation "View evidence" links | `cycles-dashboard` v0.1.25.62 (Evidence page) / v0.1.25.63 (reservation evidence links); `cycles-server` v0.1.25.37+ for `include=evidence`; `cycles-server-events` v0.1.25.14+ to sign envelopes |
