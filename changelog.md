...
| `cycles-spring-ai-starter` | 0.2.0 | 2026-05-12 | Spring Boot 3.5+, Spring AI 1.0+/1.1+, Java 21 — adds streaming chat gating (`CyclesBudgetStreamAdvisor`), real `ChatResponse.Usage` extraction on commit, prompt-based per-call estimates (`estimate-from-prompt`), `ToolCallback` decoration via `CyclesToolGate`, and a chat-client `ObservationConvention` (`CyclesChatClientObservationConvention`) for Cycles attribution on traces. All v0.1.0 known-limitations addressed. Companion to `cycles-spring-boot-starter`. | [CHANGELOG](https://github.com/runcycles/cycles-spring-ai-starter/blob/main/CHANGELOG.md) |
