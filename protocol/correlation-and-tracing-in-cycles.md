...
- **`correlation_id`** is set by the server — a deterministic hash over `(tenant_id, scope, action_kind_or_risk_class, window, window_key)`. Because the hash is deterministic, every event produced for the same tenant/scope/kind/window cluster carries the same value, which is what lets you JOIN threshold-alert → trip → reset chains and `observed_denied` ↔ `reservation.denied` pairs. It is scoped to the event stream only — it does not appear on responses or audit rows.
